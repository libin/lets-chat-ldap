...
Ldap.findOrCreateFromLDAP = function(options, core, ldapEntry, callback) {
    ...
    var uid = ldapEntry[options.field_mappings.uid];
    User.findOne({ uid: Array.isArray(uid) ? uid[0] : uid }, function (err, user) {
        if (err) {
            return callback(err);
        }
        if (!user) {
...
Ldap.createLdapUser = function(core, options, ldapEntry, callback) {
    ...
    var uid = ldapEntry[field_mappings.uid];
    var username = ldapEntry[field_mappings.username] || ldapEntry[field_mappings.uid];
    var data = {
